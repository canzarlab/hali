...
int symdiff_size(const list<string> &A, const list<string> &B)
{
    vector<string> D(A.size()+B.size());
    vector<string>::iterator dit = set_symmetric_difference(A.begin(), A.end(), B.begin(), B.end(), D.begin());
...
int main(int argc, char * const argv[]) {
    ...
    ap.refOption("c", "Objective based on GRF (j) or symmetric difference (s)", c, false);
	...
	int tot1 = 0;
	    ...
	    ListDigraph::InArcIt a1(t1.get_graph(), i);
            ...
            if (a1 != INVALID && o1 != INVALID)
	    	tot1 += t1.clade(i).size();
        }
	int tot2 = 0;
        count = 1;
        for (ListDigraph::NodeIt i(t2.get_graph()); i != INVALID; ++i) {
            ...
            count++;
	    ListDigraph::InArcIt a2(t2.get_graph(), i);
            ListDigraph::OutArcIt o2(t2.get_graph(), i);
            if (a2 != INVALID && o2 != INVALID)
	    	tot2 += t2.clade(i).size();
        }
        ...
        for (ListDigraph::NodeIt i(t1.get_graph()); i != INVALID; ++i) {
            ...
            {                
                ...
                if (a1 != INVALID && a2 != INVALID && o1 != INVALID && o2 != INVALID) //t1.clade(i).size() > 1 && t2.clade(j).size() > 1)                    
                    if (c=="s")
                        w = t1.clade(i).size() + t2.clade(j).size() - symdiff_size(t1.clade(i), t2.clade(j));
                    else if (c=="j")
                        w = 2 - Jaccard_weight(t1.clade(i), t2.clade(j), k);
                    else {
                        cerr << "invalid objectiv specified" << endl;
                        exit(1);
                    }
                ss << w << "\t";
	...
	std::ofstream toffset;
	toffset.open("offset_t1_t2");
	toffset << tot1 << " " << tot2 << endl;
	toffset.close();
